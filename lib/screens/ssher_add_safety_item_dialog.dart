...
class SsherItemInput {
  ...
  final String linkedWbsId;
  final String linkedCostItemId;
  final String linkedRequirementId;
...
class _AddSsherItemDialogState extends State<AddSsherItemDialog> {
  ...
  late TextEditingController _traceabilityNotesCtrl;
  late String _department;
  late String _riskLevel;
  late String _selectedWbsId;
  late String _selectedRequirementId;

  @override
  void initState() {
    super.initState();
    _memberCtrl =
        TextEditingController(text: widget.initialData?.teamMember ?? '');
    _concernCtrl =
        TextEditingController(text: widget.initialData?.concern ?? '');
    _mitigationCtrl =
        TextEditingController(text: widget.initialData?.mitigation ?? '');
    _traceabilityNotesCtrl = TextEditingController(
        text: widget.initialData?.traceabilityNotes ?? '');
    _department = widget.initialData?.department ?? 'Operations';
    _riskLevel = widget.initialData?.riskLevel ?? 'High';
    _selectedWbsId = widget.initialData?.linkedWbsId ?? '';
    _selectedRequirementId = widget.initialData?.linkedRequirementId ?? '';
  ...
  void dispose() {
    ...
    _traceabilityNotesCtrl.dispose();
    super.dispose();
  }
  ...
  InputDecoration _inputDecoration(
    ...
    final outlineColor = colorScheme.outline
        .withOpacity(theme.brightness == Brightness.light ? 0.2 : 0.4);
    ...
    return InputDecoration(
      ...
      fillColor: Color.alphaBlend(
          ...
          colorScheme.surfaceContainerHighest
              .withOpacity(theme.brightness == Brightness.light ? 0.65 : 0.35)),
                  ...
                  return Column(children: [
                    ...
                    ExpandingTextFormField(
                      ...
                      decoration: _inputDecoration(
                          widget.concernLabel, theme, colorScheme),
                      style: theme.textTheme.bodyMedium
                    ...
                    ExpandingTextFormField(
                      ...
                      minLines: 3,
                      decoration: _inputDecoration(
                          widget.mitigationLabel, theme, colorScheme),
                      style: theme.textTheme.bodyMedium
                          ?.copyWith(color: colorScheme.onSurface),
                      validator: (v) =>
                          (v == null || v.trim().isEmpty) ? 'Required' : null,
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Optionally link this SSHER item to WBS, cost items, or requirements for traceability.',
                            style: theme.textTheme.bodySmall
                                ?.copyWith(color: colorScheme.onSurfaceVariant),
                          ),
                          const SizedBox(height: 12),
                          ..._traceabilityRows(theme, colorScheme),
                          const SizedBox(height: 12),
                          ExpandingTextFormField(
                            controller: _traceabilityNotesCtrl,
                            minLines: 2,
                            decoration: _inputDecoration(
                                'Traceability Notes', theme, colorScheme),
                            style: theme.textTheme.bodyMedium
                                ?.copyWith(color: colorScheme.onSurface),
                          ),
                        ],
    ...
    return [
      Expanded(
        child: DropdownButtonFormField<String>(
          ...
          decoration:
              _inputDecoration(widget.departmentLabel, theme, colorScheme),
      ...
      Expanded(
        child: VoiceTextFormField(
          ...
          decoration:
              _inputDecoration(widget.teamMemberLabel, theme, colorScheme),
          style: theme.textTheme.bodyMedium
              ?.copyWith(color: colorScheme.onSurface),
      ...
      Expanded(
        child: DropdownButtonFormField<String>(
          ...
          decoration:
              _inputDecoration(widget.riskLevelLabel, theme, colorScheme),
          dropdownColor: colorScheme.surface,
          style: theme.textTheme.bodyMedium
              ?.copyWith(color: colorScheme.onSurface),
        ),
  ...
  List<Widget> _traceabilityRows(
    ...
    return [
      dropdown(
        label: 'Linked WBS Item',
        value: _selectedWbsId,
        items: [
          const DropdownMenuItem(value: '', child: Text('None')),
          for (final item in widget.wbsItems)
            DropdownMenuItem(
                value: item.id,
                child: Text('${item.wbsCode.isNotEmpty ? '${item.wbsCode} - ' : ''}${item.title}')),
        ],
        onChanged: (v) => setState(() => _selectedWbsId = v ?? ''),
      ),
      dropdown(
        ...
        items: [
          const DropdownMenuItem(value: '', child: Text('None')),
          for (final item in widget.requirementItems)
            DropdownMenuItem(
                value: item.id,
                child: Text(item.description.length > 80
                    ? '${item.description.substring(0, 80)}...'
                    : item.description)),
        ],
        onChanged: (v) => setState(() => _selectedRequirementId = v ?? ''),
      ),
    ];
  }
  ...
  void _save() {
    ...
    Navigator.pop(
      ...
      SsherItemInput(
        ...
        linkedWbsId: _selectedWbsId,
        linkedRequirementId: _selectedRequirementId,
        traceabilityNotes: _traceabilityNotesCtrl.text.trim(),
      ),
    );
  }
}
