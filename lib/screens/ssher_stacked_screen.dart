...
class _SsherStackedScreenState extends State<SsherStackedScreen>
  ...
  Future<void> _editEntry(SsherEntry entry) async {
    ...
    final input = await showDialog<SsherItemInput>(
      context: context,
      builder: (ctx) {
        final data = ProjectDataHelper.getData(ctx);
        return AddSsherItemDialog(
          ...
          wbsItems: data.wbsTree,
          requirementItems: data.frontEndPlanning.requirementItems,
          initialData: SsherItemInput(
            ...
            linkedWbsId: entry.linkedWbsId,
            linkedCostItemId: entry.linkedCostItemId,
            linkedRequirementId: entry.linkedRequirementId,
    ...
    setState(() {
      ...
      entry.linkedWbsId = input.linkedWbsId;
      entry.linkedCostItemId = input.linkedCostItemId;
      entry.linkedRequirementId = input.linkedRequirementId;
      entry.traceabilityNotes = input.traceabilityNotes;
    });
    await _saveEntries();
  }
  ...
  Future<void> _addEntry(_SsherCategory category, SsherItemInput input) async {
    final entry = SsherEntry(
      ...
      linkedWbsId: input.linkedWbsId,
      linkedCostItemId: input.linkedCostItemId,
      linkedRequirementId: input.linkedRequirementId,
      traceabilityNotes: input.traceabilityNotes,
    );
  ...
  Future<void> _downloadAll() async {
    ...
    if (allowCsv) {
      final csv = SsherExportHelper.allEntriesToCsv(
        map,
        wbsItems: projectData.wbsTree,
        costItems: projectData.costEstimateItems,
      );
      await SsherExportHelper.downloadCsv(csv, 'ssher_all_categories.csv');
    } else {
      await SsherExportHelper.exportAllToPdf(
        map,
        wbsItems: projectData.wbsTree,
        costItems: projectData.costEstimateItems,
      );
            ...
            children: [
              ...
              if (isMobile)
                ...
                ElevatedButton.icon(
                  ...
                  label: Text(
                      allowCsv ? 'Download All (CSV)' : 'Download All (PDF)'),
      ...
      child: Row(
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Row(
              children: [
                Icon(Icons.add_circle, size: 18, color: _Palette.primary),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    PopupMenuButton<String>(
                      icon: const Icon(Icons.more_vert,
                          color: _Palette.outline, size: 22),
                ...
                if (entry.linkedWbsId.isNotEmpty ||
                    entry.linkedCostItemId.isNotEmpty ||
                    entry.linkedRequirementId.isNotEmpty ||
                    entry.traceabilityNotes.isNotEmpty) ...[
                  const SizedBox(height: 14),
                  Container(
                    width: double.infinity,
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(
                      color: _Palette.primaryFixed.withValues(alpha: 0.35),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (entry.linkedWbsId.isNotEmpty)
                          Text('WBS: ${entry.linkedWbsId}',
                              style: const TextStyle(
                                  fontSize: 12,
                                  color: _Palette.onSurfaceVariant)),
                        if (entry.linkedCostItemId.isNotEmpty)
                          Text('Cost Item: ${entry.linkedCostItemId}',
                              style: const TextStyle(
                                  fontSize: 12,
                                  color: _Palette.onSurfaceVariant)),
                        if (entry.linkedRequirementId.isNotEmpty)
                          Text('Requirement: ${entry.linkedRequirementId}',
                              style: const TextStyle(
                                  fontSize: 12,
                                  color: _Palette.onSurfaceVariant)),
                        if (entry.traceabilityNotes.isNotEmpty) ...[
                          const SizedBox(height: 6),
                          Text(
                            entry.traceabilityNotes,
                            style: const TextStyle(
                                fontSize: 12,
                                color: _Palette.onSurface,
                                height: 1.4),
                  ...
                  child: Row(
                    children: [
                      ...
                      Container(
                        ...
                        height: 32,
                        decoration: BoxDecoration(
                          color: isUnassigned ? _Palette.surfaceDim : avatarBg,
  ...
  Widget _buildEmptyState(Color accent, String catLabel) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border:
            Border.all(color: _Palette.surfaceVariant.withValues(alpha: 0.5)),
      ),
      child: Column(
        children: [
  ...
  Widget _buildSaveContinueButton() {
    return Padding(
      ...
      child: SizedBox(
        ...
        height: 56,
        child: _ScaleOnTap(
          onTap: () => PlanningPhaseNavigation.goToNext(context, 'ssher'),
          child: Container(
            decoration: BoxDecoration(
  ...
  Future<void> _exportPdf() async {
    ...
    await PdfExportHelper.exportScreenPdf(
      ...
      sections: [
        ...
        PdfSection.text(
            'Notes',
            projectData.planningNotes['planning_ssher_stacked_notes'] ??
                'No data recorded.'),
      ],
    );
  }
