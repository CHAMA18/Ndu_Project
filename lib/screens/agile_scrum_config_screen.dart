...
const List<String> _structureOptions = [
  'Cross-functional',
  'Component Team',
  'Feature Team',
  'Matrix',
...
String _defaultFor(String key) {
  switch (key) {
    case 'planning_duration': return '2 hrs';
    case 'daily_scrum_time': return '09:00';
    case 'daily_scrum_duration': return '15 min';
    case 'review_duration': return '1 hr';
    case 'retro_duration': return '1 hr';
    case 'refinement_duration': return '1 hr';
    default: return '';
  }
}
...
class _AgileScrumConfigScreenState extends State<AgileScrumConfigScreen> {
  ...
  static const List<_FieldConfig> _fields = [
    _FieldConfig(key: 'po', label: 'Product Owner', hint: 'Name of the Product Owner'),
    _FieldConfig(key: 'sm', label: 'Scrum Master', hint: 'Name of the Scrum Master'),
    _FieldConfig(key: 'team', label: 'Development Team', hint: 'Team composition and size'),
  ];

  static const List<_DoaField> _doaFields = [
    _DoaField(key: 'definitionOfReady', label: 'Definition of Ready',
        hint: 'Conditions that must be met before a story can be taken into a sprint. E.g. clear acceptance criteria, estimated, dependencies identified, UX reviewed.'),
    _DoaField(key: 'definitionOfDone', label: 'Definition of Done',
        hint: 'Conditions that must be met for a story to be considered complete. E.g. code reviewed, tested, documented, deployed to staging.'),
    _DoaField(key: 'teamValues', label: 'Team Values',
        hint: 'Core team values and working norms. E.g. transparency, respect, continuous improvement, psychological safety.'),
    _DoaField(key: 'coreHours', label: 'Core Hours',
        hint: 'Overlap hours when the full team is available. E.g. 10:00-15:00 ET.'),
  ];
  ...
  void initState() {
    ...
    for (final key in [
      'planning_duration', 'daily_scrum_time', 'daily_scrum_duration',
      'review_duration', 'retro_duration', 'refinement_duration',
    ]) {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (rawAgreements != null) {
        ...
        _agreements = [
          _WorkingAgreement(category: 'Communication', description: 'Daily standup at scheduled time'),
          _WorkingAgreement(category: 'Code Quality', description: 'All code must be peer reviewed'),
          _WorkingAgreement(category: 'Collaboration', description: 'Async communication via project channel'),
          _WorkingAgreement(category: 'Meeting', description: 'Arrive on time; keep discussions focused'),
        ];
      }
  ...
  void _rebuildAgreementCtrls() {
    for (final c in _agreementCtrls.values) { c.dispose(); }
    for (final c in _agreementCatCtrls.values) { c.dispose(); }
  ...
  Future<void> _performSave() async {
    ...
    try {
      ...
      data['workingAgreements'] = _agreements.map((a) => {
        'id': a.id,
        'category': a.category,
          ...
          children: [
            DraggableSidebar(
              ...
              child: const InitiationLikeSidebar(
                  activeItemLabel: 'Agile Delivery Model - Scrum Configuration'),
            ),
            Expanded(
              child: Stack(
                children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        LaunchPhaseNavigation(
                          backLabel: PlanningPhaseNavigation.backLabel('agile_scrum_config'),
                          nextLabel: PlanningPhaseNavigation.nextLabel('agile_scrum_config'),
                          onBack: () => PlanningPhaseNavigation.goToPrevious(
                              context, 'agile_scrum_config'),
      ...
      child: Column(
        ...
        children: [
          const Text('Scrum Roles',
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: _kHeadline)),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _buildDropdownField('Sprint Planning', 'planning_duration', _durationOptions),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(
                child: _buildTextField('Daily Scrum Time', 'daily_scrum_time',
                    'e.g. 09:00'),
              ),
          ...
          const SizedBox(height: 12),
          _buildDropdownField('Sprint Review', 'review_duration', _durationOptions),
          const SizedBox(height: 12),
          _buildDropdownField('Sprint Retrospective', 'retro_duration', _durationOptions),
          const SizedBox(height: 12),
          _buildDropdownField('Backlog Refinement', 'refinement_duration', _durationOptions),
        ],
      ),
    );
  }
  ...
  Widget _buildDropdownField(String label, String key, List<String> options) {
    return Column(
      ...
      children: [
        Text(label,
            style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: _kHeadline)),
        const SizedBox(height: 6),
        DropdownButtonFormField<String>(
          value: options.contains(_selectedValues[key]) ? _selectedValues[key] : options[0],
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          ..._agreements.asMap().entries.map((e) => _buildAgreementRow(e.key, e.value)),
        ],
      ),
    );
  }
        ...
        children: [
          SizedBox(
            ...
            child: DropdownButtonFormField<String>(
              value: ['Communication', 'Code Quality', 'Collaboration', 'Meeting']
              ...
              items: const [
                DropdownMenuItem(value: 'Communication', child: Text('Communication', style: TextStyle(fontSize: 11))),
                DropdownMenuItem(value: 'Code Quality', child: Text('Code Quality', style: TextStyle(fontSize: 11))),
                DropdownMenuItem(value: 'Collaboration', child: Text('Collaboration', style: TextStyle(fontSize: 11))),
                DropdownMenuItem(value: 'Meeting', child: Text('Meeting', style: TextStyle(fontSize: 11))),
              ],
              onChanged: (v) {
                if (v != null) {
...
class _FieldConfig {
  ...
  final String label;
  final String hint;
  const _FieldConfig({required this.key, required this.label, required this.hint});
}
