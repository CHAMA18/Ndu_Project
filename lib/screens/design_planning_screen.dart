...
class _DesignPlanningScreenState extends State<DesignPlanningScreen> {
  ...
  final Set<String> _dismissedAiMappingSpecs = <String>{};
  List<AiSpecScopeMapping> _aiSpecScopeSuggestions = const [];
    ...
    return Column(
      ...
      children: [
        ...
        InnerPageNavigationHint(
          ...
          description:
              'This page has 15 guided sections. Complete each section at your own pace.',
  ...
  Widget _buildRequirementsSection(List<String> owners) {
    ...
    final aiUnmappedSuggestions = _unacceptedAiMappings(specificationOptions);
    return _buildGuidedSectionCard(
      sectionId: 'requirements',
      ...
      child: Column(
        children: [
          ...
          if (aiUnmappedSuggestions.isNotEmpty) ...[
            Container(
              width: double.infinity,
                ...
                children: [
                  Row(
                    children: [
                      ...
                      TextButton.icon(
                        onPressed: _isGenerating('requirements_ai_mapping')
                            ? null
                            : () => _generateAiSpecMappings(
                                projectData, specificationOptions),
                        icon: _isGenerating('requirements_ai_mapping')
                            ? const SizedBox(
                                width: 14,
                                height: 14,
                                child:
                        ...
                        label: Text(_aiSpecScopeSuggestions.isEmpty
                            ? 'Suggest mappings'
                            : 'Refresh suggestions'),
                      ),
                    ],
                  ...
                  Text(
                    '${aiUnmappedSuggestions.length} AI suggestion(s) are waiting for accept/reject/update review.',
                    style:
                        const TextStyle(fontSize: 12, color: Color(0xFF1E3A8A)),
                  ),
                  const SizedBox(height: 10),
                  ...aiUnmappedSuggestions
                      .take(5)
                      .map((suggestion) => _AiMappingSuggestionCard(
                            suggestion: suggestion,
                            onAccept: () => _acceptAiMappingSuggestion(
                                suggestion, specificationOptions, owners),
                            onReject: () =>
                                _rejectAiMappingSuggestion(suggestion),
                            onUpdate: () => _acceptAiMappingSuggestion(
                              suggestion,
                              specificationOptions,
                              owners,
                              markAsDraftOnly: true,
                            ),
                          )),
                ],
              ),
            ...
            Align(
              ...
              child: TextButton.icon(
                ...
                icon: _isGenerating('requirements_ai_mapping')
                    ...
                    : const Icon(Icons.auto_awesome, size: 16),
                label: const Text('Suggest mappings with AI'),
              ),
            ),
          ],
          _SubHeader(
  ...
  List<AiSpecScopeMapping> _unacceptedAiMappings(
    List<_SpecificationOption> specificationOptions,
  ) {
    final mappedTexts = _document.requirements
        .map((item) => item.requirementText.trim().toLowerCase())
        .where((value) => value.isNotEmpty)
        .toSet();
    final specTitles = specificationOptions
        .map((item) => item.title.trim().toLowerCase())
        .where((value) => value.isNotEmpty)
        .toSet();
    return _aiSpecScopeSuggestions.where((suggestion) {
      final specKey = suggestion.specTitle.trim().toLowerCase();
      if (specKey.isEmpty) return false;
      if (!specTitles.contains(specKey)) return false;
      if (_dismissedAiMappingSpecs.contains(specKey)) return false;
      return !mappedTexts.contains(specKey);
    }).toList();
  }

  Future<void> _generateAiSpecMappings(
    ProjectDataModel data,
    List<_SpecificationOption> specificationOptions,
  ) async {
    _aiGenerating['requirements_ai_mapping'] = true;
    if (mounted) setState(() {});
    try {
      final specTitles = specificationOptions
          ...
          .toSet()
          .toList();
      final wbsTitles = data.wbsTree
          .map((item) => item.title.trim())
          .where((value) => value.isNotEmpty)
          .toSet()
          .toList();
      if (specTitles.isEmpty || wbsTitles.isEmpty) {
        _showToast(
            'Add specification items and WBS titles first before requesting AI mappings.');
        return;
      }
      final contextText = ProjectDataHelper.buildExecutivePlanContext(
        data,
        sectionLabel: 'Requirements to Design Mapping',
      );
      final suggestions = await OpenAiServiceSecure().generateSpecScopeMappings(
        context: contextText,
      ...
      if (!mounted) return;
      setState(() {
        _dismissedAiMappingSpecs.clear();
        _aiSpecScopeSuggestions = suggestions;
      });
      _showToast(suggestions.isEmpty
          ? 'AI did not find any confident mapping suggestions.'
          : 'AI mapping suggestions generated. Review and accept the ones you want.');
    } catch (error) {
      _showToast('Failed to generate AI mapping suggestions: $error');
    } finally {
      _aiGenerating['requirements_ai_mapping'] = false;
      if (mounted) setState(() {});
    }
  }

  void _acceptAiMappingSuggestion(
    AiSpecScopeMapping suggestion,
    List<_SpecificationOption> specificationOptions,
    List<String> owners, {
    bool markAsDraftOnly = false,
  }) {
    final selected = specificationOptions.firstWhere(
      (item) =>
          item.title.trim().toLowerCase() ==
          suggestion.specTitle.trim().toLowerCase(),
      orElse: () => _SpecificationOption(id: '', title: suggestion.specTitle),
    );
    final designArea =
        selected.area.isNotEmpty ? selected.area : selected.discipline;
    final mapping = DesignRequirementMapping(
      requirementId: selected.id,
      requirementText:
          selected.title.isNotEmpty ? selected.title : suggestion.specTitle,
      designResponse: markAsDraftOnly
          ? 'AI suggested WBS alignment: ${suggestion.wbsItemTitle}'
          : suggestion.wbsItemTitle,
      designArea: designArea,
      owner: selected.owner.isNotEmpty
          ? selected.owner
          : (owners.isNotEmpty ? owners.first : ''),
      status: markAsDraftOnly ? 'Draft' : 'Planned',
      linkedArtifact: selected.referenceLink,
      verificationMethod: suggestion.rationale,
    );
    setState(() {
      _document.requirements.add(mapping);
      _dismissedAiMappingSpecs.add(suggestion.specTitle.trim().toLowerCase());
    });
    _queueSave();
    _showToast(markAsDraftOnly
        ? 'AI suggestion added as a draft mapping for manual refinement.'
        : 'AI suggestion accepted into the mapping table.');
  }

  void _rejectAiMappingSuggestion(AiSpecScopeMapping suggestion) {
    setState(() {
      _dismissedAiMappingSpecs.add(suggestion.specTitle.trim().toLowerCase());
    });
    _showToast('AI suggestion dismissed.');
  }
...
class _AiMappingSuggestionCard extends StatelessWidget {
  const _AiMappingSuggestionCard({
  ...
  final AiSpecScopeMapping suggestion;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            '${suggestion.specTitle} → ${suggestion.wbsItemTitle}',
            style: const TextStyle(
                fontSize: 13,
                fontWeight: FontWeight.w700,
          ...
          const SizedBox(height: 6),
          Text(
            'Confidence ${(suggestion.confidence * 100).toStringAsFixed(0)}% • ${suggestion.rationale}',
            style: const TextStyle(
